            ...
            for f in range(F):
                for j in range(0, H_out * stride, stride):
                    for k in range(0, W_out * stride, stride):
                        x_slice = x_padded[i, :, j:j+HH, k:k+WW]
                        out[i, f, j//stride, k//stride] = torch.sum(x_slice * w[f]) + b[f]
            ...
            for f in range(F):
                for j in range(0, H, stride):
                    for k in range(0, W, stride):
                        dout_slice = dout[i, f, j//stride, k//stride]
                        dx_padded[i, :, j:j+HH, k:k+WW] += dout_slice * w[f]
                        dw[f] += dout_slice * x_padded[i, :, j:j+HH, k:k+WW]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        ####################################################################
        cache = (x, pool_param)
        return out.type(torch.float64), cache
            ...
            for j in range(C):
                for k in range(H_out):
                    for l in range(W_out):
                        ...
                        x_max = torch.max(x_slice)
                        dx[i, j, k*stride:k*stride+pool_height, l*stride:l*stride+pool_width] += dout[i, j, k, l] * (x_slice == x_max)
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out, cache2 = ReLU.forward(out)
        out, cache3 = FastMaxPool.forward(out, pool_param)
        out, cache4 = Linear.forward(out, W2, b2)
        out, cache5 = ReLU.forward(out)
        out, cache6 = Linear.forward(out, W3, b3)
        scores = out
        ...
        dout, grads['W3'], grads['b3'] = Linear.backward(dout, cache6)
        dout = ReLU.backward(dout, cache5)
        dout, grads['W2'], grads['b2'] = Linear.backward(dout, cache4)
        dout = FastMaxPool.backward(dout, cache3)
        dout = ReLU.backward(dout, cache2)
        dout, grads['W1'], grads['b1'] = Conv.backward(dout, cache1)
