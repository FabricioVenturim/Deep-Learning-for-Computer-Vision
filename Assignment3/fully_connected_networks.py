...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dx = dout.mm(w.t()).reshape(x.shape) # (N, M) * (M, D) = (N, D) -> (N, d1, ..., d_k)
        dw = x.reshape(x.shape[0], -1).t().mm(dout) # (D, N) * (N, M) = (D, M)
        db = dout.sum(dim=0) # (M, )
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Backward into second layer
        da1, grads['W2'], grads['b2'] = Linear.backward(dscores, cache2)

        # Backward into first layer
        dx, grads['W1'], grads['b1'] = Linear_ReLU.backward(da1, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers - 1):
            self.params['W' + str(i + 1)] = torch.normal(0, weight_scale, (input_dim, hidden_dims[i]), dtype=dtype, device=device)
            self.params['b' + str(i + 1)] = torch.zeros(hidden_dims[i], dtype=dtype, device=device)
            input_dim = hidden_dims[i]

        # Initialize weights and biases for output layer
        self.params['W' + str(self.num_layers)] = torch.normal(0, weight_scale, (input_dim, num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        # Forward into output layer
        scores, cache = Linear.forward(a, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        caches.append(cache)
        ...
        # Backward into output layer
        da, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = Linear.backward(dscores, caches.pop())

        # Backward into hidden layers
        for i in range(self.num_layers - 1):
            da, grads['W' + str(self.num_layers - i - 1)], grads['b' + str(self.num_layers - i - 1)] = Linear_ReLU.backward(da, caches.pop())
